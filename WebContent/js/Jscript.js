...
function loadProducts()
{
	...
	$.ajax({
		...
		success : function(responseText) {
			...
			//alert(obj[0].data.length);
			//alert(obj[0].data[0].productname);
			var productTable="<table><tr class='tile1' style='height:30px;'><td>Select</td><td>Brand</td><td>Product Name</td><td> Price</td><td>select</td></tr>";
			for(var i=0;i<obj[0].data.length;i++)
			{
				...
				if(i%2 == 0)
				{
					productTable = productTable + "<tr class='tile' style='height:100px;'><td><img id="+obj[0].data[i].productid+" src="+filename+" height='100' width='100' onclick='Addtocart("+pid+","+pquantity+","+pprice+");'/></td><td>"+obj[0].data[i].brandname+"</td><td>"+obj[0].data[i].productname+"</td><td id="+pr_id+">"+obj[0].data[i].price+"</td><td><button type='button' onclick='Addtocart("+pid+","+pquantity+","+pprice+")'>Add to Cart</button></td>" +
					"</tr>";
					}
				else
				{
