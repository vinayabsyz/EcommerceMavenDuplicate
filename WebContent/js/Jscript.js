	...
	$.ajax({
		...
		success : function(responseText) {
			...
			for(var i=0;i<obj[0].data.length;i++)
			{
				...
				if(i%2 == 0)
				{
					productTable = productTable + "<tr class='tile' style='height:100px;'><td><img id="+obj[0].data[i].productid+" src="+filename+" height='100' width='100'/></td><td>"+obj[0].data[i].brandname+"</td><td>"+obj[0].data[i].productname+"</td><td id="+pr_id+">"+obj[0].data[i].price+"</td><td><button class="btn btn-success" type='button' onclick='Addtocart("+pid+","+pquantity+","+pprice+")'>Add to Cart</button></td>" +
					"</tr>";
					}
				else
				{
